doSharedMuncherLogic = function(grid, bug, i, j) { // this logic in here is actually shared AGENT knowledge
    bug.done = true;
    // TODO add more to this function once different bug types are actually Classes
}

doRedmuncherLogic = function(grid, redmuncher, i, j) {
    doSharedMuncherLogic(grid, redmuncher, i, j);
    ...
    if (redmuncher.stomach > 0) { // RIP redmuncher if empty stomach, otherwise it moves instead of dies
      ...
      if (destinationCell.agent === null) {
        ...
        if (willReproduce) {
          ...
          numRedmunchers++; // it seems unfortunate that this logic must live here for now
